...
function TemplateEditorDialog({
  ...
  return (
    <div
      ref={backdropRef}
      className={open ? "fixed inset-0 z-50 flex items-stretch justify-center bg-overlay p-3 sm:p-4 lg:p-6" : "hidden"}
      data-testid="game-template-editor-dialog"
      aria-hidden={!open}
      onPointerDown={(event) => {
        pointerStartedOnBackdropRef.current = event.target === backdropRef.current;
      }}
      onClick={(event) => {
        if (pointerStartedOnBackdropRef.current && event.target === backdropRef.current) {
          onClose();
        }
    ...
    >
      <section
        role="dialog"
        aria-modal="true"
        aria-labelledby="game-template-editor-dialog-title"
        className="flex min-h-0 w-full max-w-7xl flex-col overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl"
      >
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-border px-5 py-4">
          <div className="min-w-0">
            <div className="qf-shell-folio">Template Workshop</div>
            <h2 id="game-template-editor-dialog-title" className="mt-1 text-xl font-semibold text-text">Game template editor</h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-text-muted">
              Closing hides this panel and keeps unsaved edits in memory. Use the editor's load, new, delete, or save actions when you intentionally want to change template state.
            </p>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg bg-surface-alt px-3 py-2 text-sm font-medium text-text transition-colors hover:bg-border"
            aria-label="Close game template editor"
          >
            Close
          </button>
        </div>
...
export default function GamesWorkspace({
  ...
  async function reloadTemplates(preferredTemplateId?: string) {
    try {
      ...
      const fallbackTemplateId = data.templates[0]?.templateId || "";
      setTemplates(data.templates);
      setSelectedTemplateId((previous) => (preferredTemplateId ?? previous) || fallbackTemplateId);
      setEditorTemplateId((previous) => previous || preferredTemplateId || fallbackTemplateId);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    async function loadTemplates() {
      try {
        ...
        const fallbackTemplateId = data.templates[0]?.templateId || "";
        setTemplates(data.templates);
        setSelectedTemplateId((previous) => previous || fallbackTemplateId);
        setEditorTemplateId((previous) => previous || fallbackTemplateId);
      } catch (err) {
        if (!cancelled) {
          setError(err instanceof Error ? err.message : "Failed to load game templates");
        }
          ...
          <div className="border-b border-border/50 px-4 py-4">
            {!sessionId ? (
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="text-xs leading-5 text-text-muted">
                  Template editing opens in a separate workshop panel so setup and roster controls stay compact.
                </div>
              </div>
            )}
          </div>
