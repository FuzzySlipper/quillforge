...
export default function CouncilWorkspace({
  ...
  const isEmpty = members.length === 0 && messages.length === 0;

  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className={`border-b border-border/70 ${isEmpty ? "px-6 py-3" : "px-6 py-5"}`}>
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0">
            <div className="qf-shell-folio">Council Chamber</div>
            <h1 className={`qf-shell-title ${isEmpty ? "mt-0" : "mt-1"}`}>Multi-voice advisory synthesis</h1>
            {!isEmpty && (
        ...
        <div className={`flex flex-wrap gap-2 ${isEmpty ? "mt-2" : "mt-4"}`}>
      ...
      {isEmpty ? (
        <div className="flex min-h-0 flex-1 items-center justify-center p-4">
          <section className="qf-shell-card qf-shell-card--sunken max-w-lg px-6 py-6 text-center">
            <div className="qf-shell-folio">Active Roster</div>
            <p className="mt-2 text-sm leading-6 text-text-muted">
              No council members are configured yet. Open <button type="button" onClick={onOpenCouncilConfig} className="text-accent underline underline-offset-2 hover:text-accent-hover">Advisor Settings</button> to create the panel.
            </p>
            {error && (
              <div className="mt-4 rounded-lg border border-danger-border bg-danger-soft px-4 py-3 text-sm text-danger-text">
                {error}
              </div>
            )}
          </section>
