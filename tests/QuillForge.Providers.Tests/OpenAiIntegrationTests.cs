...
/// Skipped if OPENAI_API_KEY is not set. Use small token budgets.
///
...
public class OpenAiIntegrationTests
{
    ...
    private static string? GetApiKey() => Environment.GetEnvironmentVariable("OPENAI_API_KEY");
    ...
    public async Task SimpleCompletion_ReturnsText()
    {
        ...
        var service = registry.GetCompletionService("openai");
    ...
    public async Task ToolCall_ModelCallsDefinedTool()
    {
        ...
        Assert.True(toolCalls.Count > 0, "Expected at least one tool call");
